...
struct CartographySearchView<InitialView: View, ResultsView: View>: View {
    ...
    var body: some View {
        ...
        .searchable(text: $query, placement: Constants.searchBarPlacement, prompt: "Go To...")
        .animation(.default, value: searchState)
        .onAppear {
            #if DEBUG
    ...
    private func performSearch() async {
        ...
        let context = CartographySearchService.SearchContext(
            ...
            position: position,
            dimension: dimension)
        let results = await service.search(for: query, in: context)
