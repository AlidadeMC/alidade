...
struct SearchedStructuresSectionTests {
    @Test func viewLayout() throws {
        ...
        let section = SearchedStructuresSection(
            structures: [.init(position: .zero, name: "Mineshaft")],
            viewModel: viewModel,
        ...
        let button = try sut.section().forEach(0).view(CartographyNamedLocationView.self, 0)
